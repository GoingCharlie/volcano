...
var _ = Describe("Repack HyperNode-aware constraint preservation (US-02)", Serial, func() {
	...
	Context("E25: a partly drained multi-role job stays in its surviving role's domain (US-02)", func() {
		It("plans nothing even when the sibling domain has a spare slot and passes the resource prefilter", func() {
			// E24 with rt-s1's second node left empty instead of fully occupied: an
			// empty node stays out of the receiver pool but still counts towards
			// rt-s1's cached idle, so the domain prefilter admits rt-s1 and only the
			// surviving role's anchor can refuse the split (E24's rt-s1 is filtered
			// out before the anchor is consulted). Dropping the plan-placed role from
			// the kept set splits the job here.
			job := createMovableJob(ctx, subGroupJobSpec(ctx, "e25", ptr.To(1),
				repackSubGroupTask{name: "task-a", cards: 2, reps: 1, tier: ptr.To(1)},
				repackSubGroupTask{name: "task-b", cards: 2, reps: 1, tier: ptr.To(1)}), nodes[0])
			occupy(ctx, "e25-s1", nodes[1], 6) // rt-s0: one slot, room for one role
			occupy(ctx, "e25-s2", nodes[2], 6) // rt-s1: one slot, room for one role
			// nodes[3] stays empty: rt-s1's idle saver, never a receiver.
			before := runningPodCount(ctx)

			run, err := newRun("e25", repackv1alpha1.RepackModeExecute).
				goal(npuResource).create(ctx)
			Expect(err).NotTo(HaveOccurred())
			...
			for _, pod := range runningPodsOfJob(ctx, job) {
				Expect(tierDomainOfPod(ctx, pod, 1)).To(Equal("rt-s0"),
					"the second role must follow the role already placed in rt-s0")
			}
		})
	})
