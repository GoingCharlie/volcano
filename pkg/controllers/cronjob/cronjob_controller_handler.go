...
func (cc *cronjobcontroller) handleJobError(queue workqueue.TypedRateLimitingInterface[string], key string, err error) {
	...
	klog.Warningf("Dropping cronjob<%s> out of the queue: %v because max retries has reached",
		key, err)
}
	...
	for _, job := range JobsByCronJob {
		...
		if !found && !isFinish {
			...
			cc.recorder.Eventf(cronJob, corev1.EventTypeWarning, "OrphanedJob",
				"Detected orphaned job forgotted or not managed by controller: %s (UID: %s)",
				job.Name, job.UID)
		}
	}
...
func deleteJobByApi(vcClient vcclientset.Interface, cc *batchv1.CronJob, job *batchv1.Job, recorder record.EventRecorder) bool {
	...
	if vcClient == nil || cc == nil || job == nil {
		klog.ErrorS(nil, "Invalid arguments provided to deleteJobByApi",
