...
log "创建独立测试 namespace，并为源节点添加测试标签"
...
log "通过临时 NoSchedule 污点将 source 4 卡 Pod 定位到 $SOURCE_NODE"
taint_all_except "$SOURCE_NODE" "$INITIAL_TAINT_KEY"
kubectl apply -f "$RENDERED_DIR/10-source-before.yaml" >/dev/null
wait_for_pod_ready "$SOURCE_POD_NAME"
[ "$(pod_node "$SOURCE_POD_NAME")" = "$SOURCE_NODE" ] || fail "$SOURCE_POD_NAME 未落到 $SOURCE_NODE"
[ "$(pod_target_request "$SOURCE_POD_NAME")" -eq "$TEST_POD_CARDS" ] || fail "$SOURCE_POD_NAME 未申请 $TEST_POD_CARDS 张目标卡"
clear_initial_taints

log "通过临时 NoSchedule 污点将 receiver 4 卡 Pod 定位到 $RECEIVER_NODE"
...
[ "$(pod_target_request "$RECEIVER_POD_NAME")" -eq "$TEST_POD_CARDS" ] || fail "$RECEIVER_POD_NAME 未申请 $TEST_POD_CARDS 张目标卡"
clear_initial_taints
