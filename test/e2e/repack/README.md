...
| Execute result | success, verified no-op, static PDB filtering, dynamic PDB rejections and plan/result separation, causal source-node release, concurrent reuse |
...
when every replacement is scheduled and every planned source is released by the
repack; unrelated workloads may already have reused that capacity in the terminal
snapshot.
