...
func runWithNoms(name string, noms ...repackv1alpha1.PodRelocationStatus) *repackv1alpha1.RepackRun {
	r := &repackv1alpha1.RepackRun{
		...
		Status:     repackv1alpha1.RepackRunStatus{Phase: repackv1alpha1.RepackRunning, ExecutionDeadline: func() *metav1.Time { deadline := metav1.NewTime(time.Now().Add(time.Hour)); return &deadline }()},
	}
...
func TestMatchNomination(t *testing.T) {
	t.Run("victimPodName exact wins", func(t *testing.T) {
		n := nominatorWith(runWithNoms("r1",
			repackv1alpha1.PodRelocationStatus{Namespace: "ns", PodGroupName: "g", VictimPodName: "w-0", PlannedNodeName: "n2"},
		))
	...
	t.Run("victimPodName exact remains scoped to PodGroup", func(t *testing.T) {
		n := nominatorWith(runWithNoms("r1",
			repackv1alpha1.PodRelocationStatus{Namespace: "ns", PodGroupName: "source", VictimPodName: "w-0", PlannedNodeName: "n2"},
		))
	...
	t.Run("homogeneous PodGroup when scheduling requirements hash is empty", func(t *testing.T) {
		n := nominatorWith(runWithNoms("r1",
			repackv1alpha1.PodRelocationStatus{Namespace: "ns", PodGroupName: "g", PlannedNodeName: "n1"},
		))
	...
	t.Run("no match: wrong PodGroup / namespace / bound", func(t *testing.T) {
		...
		wrongPG := repackv1alpha1.PodRelocationStatus{Namespace: "ns", PodGroupName: "other", PlannedNodeName: "n1"}
		wrongNS := repackv1alpha1.PodRelocationStatus{Namespace: "elsewhere", PodGroupName: "g", PlannedNodeName: "n1"}
...
func TestHomogeneousNominationWaitsForVictimDeletion(t *testing.T) {
	n := nominatorWith(runWithNoms("r1", repackv1alpha1.PodRelocationStatus{
		Namespace: "ns", PodGroupName: "g", VictimPodName: "old", PlannedNodeName: "n2",
	}))
	...
	tests := []struct {
	...
	}{
		{
			...
			nomination: repackv1alpha1.PodRelocationStatus{
				...
				VictimPodName: "worker-0", PlannedNodeName: "node-a",
			},
		...
		},
		{
			...
			nomination: repackv1alpha1.PodRelocationStatus{
				Namespace: "ns", PodGroupName: "old", ReplacementPodGroupName: "new",
				VictimPodName: "old-random", PlannedNodeName: "node-a",
			},
