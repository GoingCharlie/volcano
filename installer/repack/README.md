...
optional; omitting one only disables its policy. `pdbconstraint` excludes
accelerator Pods protected by a fresh, deterministic zero-disruption PDB during
planning; temporary allowance exhaustion is still handled by the Eviction API
and the existing retry loop. The `repack` Action requires at least one plugin
that provides the `domain` capability (`nodeconsolidation` today). Empty
