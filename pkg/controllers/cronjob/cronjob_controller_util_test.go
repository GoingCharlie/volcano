...
func TestByJobCreationTimestamp(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:     "both have nil creation times",
			input:    []*batchv1.Job{bNil, aNil},
			expected: []*batchv1.Job{aNil, bNil},
		},
		{
			name:     "only the first has a nil creation time",
			input:    []*batchv1.Job{aNil, bSet},
			expected: []*batchv1.Job{bSet, aNil},
		},
		{
			name:     "only the second has a nil creation time",
			input:    []*batchv1.Job{aSet, bNil},
			expected: []*batchv1.Job{aSet, bNil},
		},
		{
			name:     "both have non-nil, equal creation time",
			input:    []*batchv1.Job{bSet, aSet},
			expected: []*batchv1.Job{aSet, bSet},
		},
		{
			name:     "both have non-nil, different creation time",
			input:    []*batchv1.Job{aSetLater, bSet, aNil},
			expected: []*batchv1.Job{bSet, aSetLater, aNil},
		},
	}

	for _, testCase := range testCases {
		sort.Sort(byJobCreationTimestamp(testCase.input))
...
func TestMostRecentScheduleTime(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedRecentTime:    deltaTimeAfterTopOfTheHour(76*time.Hour + 30*time.Minute),
			expectedEarliestTime:  *deltaTimeAfterTopOfTheHour(30 * time.Minute),
			expectedTooManyMissed: fewMissed,
		},
		{
...
func TestNextScheduleTimeDuration(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			now:              *deltaTimeAfterTopOfTheHour(30*time.Hour + 30*time.Minute),
			expectedDuration: 18*time.Hour + nextScheduleDelta,
		},
		{
...
func topOfTheHour() *time.Time {
	T1, err := time.Parse(time.RFC3339, "2025-03-11T10:00:00Z")
	if err != nil {
