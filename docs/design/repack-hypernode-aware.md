...
- **计划目标不可用时由调度器接管**：节点被删除、被写满或被 predicate 挡下 → nomination 校验不过 → 调度器回退梯度分配。此时 engine 侧的 `excludedFreedNodes` 排除（原 `placement_reconcile.go:104`，喂给 `:179`/`:228` 的 receiver 计算）已不存在，**replacement 可能落回本次刚腾空的节点** → `CompareFreedNodeSets` 报 missing → Run 以 `PartiallySucceeded/BenefitNotRealized` 终态收口。该情形可观测、不会静默。
...
- **R28 Execute 期直接把计划目标写为 `SelectedNodeName`、不复算接收者（§4.2.4）**：replacement 相对 `Candidates`（`decision.go:33-51`）解析出的每条 relocation 写 `SelectedNodeName = PlannedNodeName`（一次批量落盘 `writePlacementSelection`），Nominator 镜像到 `pod.status.nominatedNodeName` 并开 placement gate（`repack-controller/pkg/nominate.go:596-610`）。**Execute 侧不调 `FeasibleRelocation`**、不做任何可行域/接收者计算，`==true` 与 `==false` 不再分流；`==true` 单元的整组单域交由调度器原生语义保证（逐 HyperNode 整单元 dry-run + 单域提交，`allocate.go:1156`、`:880-901`），engine 不再介入域选择。计划目标不可用时调度器回退梯度分配，engine 侧的 `excludedFreedNodes` 排除随之失效——replacement 可能落回本次腾空的节点，Run 以 `PartiallySucceeded/BenefitNotRealized` 终态收口；实际落点与计划不一致经 `alternativeNodePlacements` 计入 `ReasonExecutionCompletedWithAlternativePlacement`。placement 长期未绑定仍由 run 级 `ExecutionDeadline` 收口（`eviction_reconcile.go:166-168`/`:441-445` → `execution_timeout.go:119`/`:128`）。
...
**合并交互注记（wcx「PDB 重试 + eviction 截止」特性，§4.2.4/R28/E21 改写依据）**：wcx 提交 `0893b1dc3`–`e15602086`（PDB 阻塞驱逐加 deadline 重试 / 驱逐重试生命周期加固 / 零扰动 PDB plan 过滤 / run 删除时释放执行槽）合入本需求上游后，Execute 期有界性从**成员级 `ExpirationTime`** 整体替换为 **run 级 `ExecutionDeadline`**：`ExpirationTime`、`nominationTTL`、`CanExpire`、`expirePlacements` 与其消费代码全部移除（代码零残留）；`PodPlacementTimedOut` 的唯一写入者变为 `timeoutExecution`（`execution_timeout.go:119`）。新语义：engine 首次落批 eviction 时打 `Status.ExecutionDeadline = now + ExecutionTimeout`（`eviction_reconcile.go:166-168`），各 requeue 以 `capAtExecutionDeadline` 封顶（`:441-445`），deadline 到达经 `timeoutExecution`（`execution_timeout.go:43-128`）把全部非终局 placement 统一置 `PodPlacementTimedOut`（`:119`）、整 run 以 `ReasonExecutionTimedOut` 失败（`:128`）。
  ...
  - 性质：合并交互注记（行为记录，非本需求缺陷）；deadline 终局表现已由 repack e2e 验收（`placement_protocol.go`「nominates an occupied planned receiver, lets the scheduler override it, and partially succeeds」：replacement 已完成但计划节点仍被占用时，run 以 `PartiallySucceeded/BenefitNotRealized` 收口，placement gate 与 PodGroup lease 均按期释放）。
...
**设计决策注记：replacement 相「计划即选点」（§4.2.4 / R28 / §5.2.2 E21 改写依据）**：replacement 相原在 `FeasibleRelocation` 上按 `==true`/`==false` 双路径复算接收者（`==true` 整组就绪 + 整组单域原子提交），本版决定**整体移除该层**——Execute 期直接把 plan 的 `Move.ToNode` 写为 `SelectedNodeName`，落点校验交给调度器。
  ...
  - 代价（显式接受）：engine 侧的 `excludedFreedNodes` 排除随之失效——计划目标不可用时调度器回退梯度分配，而 binpack 偏好刚腾空的源域（`allocate.go:667-671` 注释），replacement 可能落回本次腾空的节点 → `PartiallySucceeded/BenefitNotRealized`。该情形可观测、不静默。`==true` 整组单域不丢（由调度器原生逐 HyperNode 整单元 dry-run + 单域提交保证）。
  - R28 本身不涉及 API 变更：`PodPlacementNominated` 仍由 Nominator 写入（`nominate.go:1446`）、由 `status/store.go:83/:129/:133` 读取。
