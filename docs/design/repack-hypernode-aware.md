...
**实现与验证状态**：两 User Story 均已实现并合入，§5 所列 UT 与 e2e 全部通过。交付后的全量检视修复合入提交 `cb018e7f2`：`gangFullyVacated` 整单元腾空判据改为集合成员相等（修正 Execute 侧 partial-evac 残留被误判为整 gang 腾空而错误清锚的 P1）、`allowedDomainsForTrial` 试放改 `defer` 恢复、调度器 job 锚点确定性 tie-break（§4.2.3 设计要点·调度器 nomination 快速路径）、独立部署安装清单补齐默认插件装配——均登记于 §7.1；经论证属设计预期或保持现状的项按 §7.2 记注记。**执行环节缺陷（本版新增 §4.2.4/R28/E21，已落地）**：全量检视发现 Execute 期 placement reconcile 逐 pod 调 `FeasibleRelocation` 把 `==true` 整组单域退化成分裂——修正为「整组就绪 + 整组单域原子提交、绝不为子集选域」规则，提交 `10f43635d` 落地。**注：wcx「PDB 重试 + eviction 截止」特性（`0893b1dc3`–`e15602086`）随后合入本需求上游，成员级 `ExpirationTime` 逃生被 run 级 `ExecutionDeadline` 截止语义取代**（§4.2.4/R28/E21 已按合并后语义改写，§7.2 记合并交互注记）；E21 已由 repack e2e 验收通过（`repack_topology_constraint.go`「==true whole-gang reconcile reproduces the plan domain」），deadline 终局表现由 `placement_protocol.go`「nominates an occupied planned receiver, lets the scheduler override it, and fails on the unrealized plan」一并验收（§7.2）。**本版决策（§4.2.4/R28 再次改写，取代上段修复）**：replacement 相不再复算接收者——Execute 期直接把 plan 的 `Move.ToNode` 写为 `SelectedNodeName`，落点校验交给调度器，决策点由三个（计划模拟 / replacement 复算 / 真实调度）收敛为两个（plan 选点、scheduler 绑定）；`==true` 整组单域改由调度器原生「逐 HyperNode 整单元 dry-run + 单域提交」保证。原「整组就绪 + 整组单域原子提交」规则、其分组键与 receiver 辅助函数一并退役，E21 断言保留但语义归属改写（见 §5.2.2）。
...
- **允许域求值细节**：**Job 单元**调 `HyperNodeGradientForJobFn(job, root, PurposeAllocate)` 一次；**SubJob 单元**调 Job 入口与 `HyperNodeGradientForSubJobFn(subJob, root, PurposeAllocate)` 两个入口、**结果取交集**——SubGroupPolicy 分支的 subJob 不继承 Job 拓扑（§4.2.2 第（1）点），Job 级网络拓扑必须由 Job 入口兜底，只调 SubJob 入口会把允许域错误地放大为全集群（subJob 自身拓扑 nil 时 `hyperNodeGradientForSubJob` abstain）。`root` 传 `ssn.HyperNodes[ClusterTopHyperNode]`——`getSearchRoot` 会把调用方 root 与 gang 允许子树**取交**，传小了会错收窄允许域。**Job 入口**传 ClusterTop 与 `allocate.go:315` 一致；**SubJob 入口真实调度是级联传逐候选域**（`allocateForJob` 外层逐域 dry-run 后、`allocateForSubJobInCandidateForest` 以该候选域为 root 调 `HyperNodeGradientForSubJobFn(subJob, hyperNodeForJob)`，`allocate.go:680`）——repack 统一传 ClusterTop，因取交满足分配律（`ClusterTop` 为全树根时 `ClusterTop∩gang允许子树` 恒等于「逐候选域收窄」的并）而**数学等价**，无需复刻级联。**`getSearchRoot` 存在三份近似实现**：`api.GetSearchRoot`（`hyper_node_info.go:925-954`，导出）、`getSearchRoot`（`network_topology_aware.go:699-724`）、`getSearchRootForGradient`（`group_topology_affinity.go:926-955`）——签名与语义逐行等价（`allocatedHyperNode==""` 返回可用子树根；否则取「可用子树 ∩ 允许包络子树」返回更窄根、无交报错），US-02 复用时须固定一份、保持行为一致。锚点语义（H1 自洽化，方案 (a)）：有剩余 pod → 收窄到允许祖先子树内；**整体腾空 → 无锚点、仅 `tier ≤ HighestTierAllowed` 生效、全集群搜索**。无锚分支仅在 `allocatedHyperNode == ""` 时进入（`isEligibleHyperNode`，`network_topology_aware.go:670-693`：`:675-677` 有锚直接放行、`:679-692` 无锚走 `hyperNodeResourceCache` 预过滤）——但计划态承载下，被整体腾空的 gang 其 pod 已改写、`SyncJobAllocatedHyperNode` 重算得**非空**锚点，无锚分支**不会自然触发**（这正是 H1 检视指出的不自洽）。故对**整体腾空单元**（该 gang 计划态无残留 pod）求允许域时，**显式临时清空 `AllocatedHyperNode` 再调梯度**（保存-恢复、纳入「试放与对称回滚」的 defer 机制；梯度 fn 直接读该字段，`network_topology_aware.go:346/:368`），使无锚分支可触发、与真实调度 evict 后 `AllocatedHyperNode == ""` 的行为一致；其余单元恒锚定。**Job 入口的「无锚」按本次调用（call）判整 job 腾空（`jobFullyVacated(scope, unit)`）**：临时清空作用的是**单元自身的** `AllocatedHyperNode`（SubJob 单元即 `subJob.AllocatedHyperNode`）；Job 入口另判**本次调用是否覆盖整 job 的计划态已分配任务**——有残留 pod（存在非本次 victim 的已分配任务）则 Job 入口保持原锚点不变，覆盖则把 Job 入口锚点**重写为「仍持有 pod 的 gang 的 LCA」**（`jobAnchorAfterEvacuation`：逐 subJob 取保留者〔未被本次调用逐出、或已被本次计划落位——`DisplacedTasks` 读计划态 `rewritten`〕的 `AllocatedHyperNode` 求 LCA；无保留者则置空、走无锚分支）。**多 subJob job 必须按 call 判、不能按单元判**：单单元只覆盖自己的 gang，按单元判恒为非整 job 腾空 → Job 入口保持驱逐前锚点（= 源域子树，其节点集恰只含被逐出的源、接收者池排除源且只收 Partial 节点）→ 每个单元都不可行、plan 恒为 nil（用户报告「没有可行的碎片整理方案」，`jobFullyVacated` 见 §7.1）。**「保留者」含已落位 gang 是必需的**：被逐出并已落位的兄弟 role 若不入账，后续 role 会被释放到**另一个** tier-1 域、把一个 hard tier-1 job 计划成跨域分裂（`GetSearchRoot`/Execute nomination 都不拦，**执行期同样不拦**——2026-09-21 探针实测：plan 拆两域后 run 报 `ExecutionCompleted`、两个 replacement 实落 `rt-s0`/`rt-s1` 两域，见 §5.2.2 E25 取证；即**规划期是这条约束的唯一关口**）；保留者 LCA 把后续 role 钉在领先者刚落位的域，与 `allocate.go:362`「Job 入口按非空 job 锚点收窄」一致。无 subJob 的 job（无 SubGroupPolicy）保留者集恒空 → 锚点置空，与既有单 role 行为逐字节一致；实现者不可把「无锚」误读为「逃逸 job 包络」——仅当整 job 被本次调用腾空时 Job 入口才不设包络。无锚路径的 `hyperNodeResourceCache` 预过滤缓存反映真实集群、不含计划内腾出容量 → 允许集**欠包含、保守**，会漏部分「计划后才可行」的域、但不产生违背约束的方案（H1 落地后保守来源仅剩「receiver 池 ∩ 资源缓存欠包含」叠加，见「已知保守性」）。
- **允许域层内排序（确定性）**：`allowed` 的**层间**顺序由插件保证——`hyperNodeGradientFn` 收集完各 tier 后显式按 tier 升序重排（`network_topology_aware.go:655-665`）；但**层内**（同一 tier 的多个域之间）顺序**不稳定**：插件以 BFS 逐层收集、子节点取自 `current.Children`（map 迭代，`:646`），同 tier 各域的入列序随运行变化。步骤 4 的 first-fit 取的是**清单中的第一个域**，不排序则**同一份输入在不同轮次会选中同 tier 的不同域、计划不可复现**（实测：同输入调用 200 次，tier-1 层出现两种序、179/21；桩级单测的梯度桩恰对子节点 `sort.Strings`，故此前未暴露）。故 adapter 在**求出 `allowed` 之后、进入 first-fit 之前**对**每个 tier 层**按 HyperNode **名升序**排序（`sortLayersByName`，`domain_relocation.go:139-147`；`allowedDomains` 的两个返回点都经它收口）。**排序只做在 adapter 侧、插件不动**：调度器的域选择由 `HyperNodeOrderFn`/评分决定、与层内序无关，改插件会波及真实调度路径；名升序对 repack 无偏好语义（步骤 4 只求一个放得下的域、不评分），仅把「同 tier 取哪个域」固定下来。单测 `TestAllowedDomains_SortsDomainsWithinTier`（构造梯度桩反向注入同层 `hD,hB,hA`，断言输出 `hA,hB,hD`）钉住。
...
- **配套改动（drain/adapter 控制流重构，实现前提）**：现有 `FeasibleRelocation` 是 per-victim 贪心循环（`snapshot_session.go:128-167`：逐 victim 经 `firstFeasibleReceiver` 找单候选 `:159-163`、`:163` 追加 `tasksPlacedByNode`），任一 victim 找不到接收者即返回 `(nil,false)`、整单元跳过（`snapshot_session.go:145-166`、`drain.go:378-385` 的 `markUnitInfeasible`），**没有**「整单元逐域试放」结构。US-02 须把它重构成**双模式**，两模式可共存于同一单元（混合 gang 单元）：
  ...
  - **Execute 侧不做任何调用（§4.2.4 决策）**：placement reconcile 不再调 `FeasibleRelocation`——它曾逐 pod 调（单 victim，`placement_reconcile.go:130`）、后又改为 `==true` 整组一次调用，两版都让 replacement 相成为独立于计划的第三个决策点。§4.2.4 已将该层整体移除：engine 只把计划目标写为 `SelectedNodeName`，落点校验由调度器承担，`==true`/`==false` 在 Execute 侧不再分流。**（检视 §3.2 的两条边界在规划期内仍然有效）**：(1) 不得引入 `if 单pod { 旧贪心 } else { 域试放 }` 的**数数**分支制造两侧不一致——`==true`/`==false` 经 `RequiresHyperNodeAllocate()` 谓词天然分流，「单 pod」只是「单元 = 1」的特例、走同一实现。(2) 单元级**全有或全无必须保留**：混合 gang 单元任一 victim 失败即整单元跳过、不保留部分成功（`snapshot_session.go:145-166` 现有语义），这是 consolidation「不释放的节点不排空」的保证，重构时不得改成「部分成功」。
...
6. **两侧一致（适用面收窄）**：规划期（drain）按步骤 3/4 保证计划目标本身落在约束允许域内；Execute 的 replacement 相**不复算**——直接把计划目标提名为替换 pod 的 `nominatedNodeName`（§4.2.4），落点校验由真实调度器承担。约束保持由「计划期选出的目标合法」+「调度器不接受非法落点」共同保证，Execute 侧不再二次判定。
...
#### 4.2.4 Execute 阶段放置：计划即选点（replacement 不复算）

**决策**：Execute 阶段 engine **不做任何可行域复算**——直接把 plan 的 `Move.ToNode` 写为替换 pod 的 `SelectedNodeName`，由 Nominator 镜像到 `pod.status.nominatedNodeName` 并开 placement gate，落点校验完全交给真实调度器。

**动因（决策点收敛）**：此前 replacement 相另做一次 `FeasibleRelocation`，与计划期用的是**同一个函数**（`adapter/snapshot_session.go:147-204`），只是快照更新。同一份计划因而存在三个可改答案的决策点——计划期模拟、replacement 复算、真实调度；这层复算带来的是「计划目标不可用时改选其它节点」的韧性，但改选结果本身即与计划不一致，且它并不消除第三层、只是把偏差从 scheduler 侧挪到 engine 侧。删除后决策点降为两个、权责单一：**plan 选点、scheduler 绑定**，engine 只负责把计划目标忠实提名出去。

**机制**：

- `reconcilePlacement` 对每个由 `Candidates`（`executor/placement/decision.go:33-51`：替换 pod 已存活且未绑定）解析出的 relocation，写 `SelectedNodeName = relocation.PlannedNodeName`，一次批量落盘（`writePlacementSelection`）。
- Nominator 读到非空 `SelectedNodeName` 后 `markPlacementNominated` + `patchNominatedNode` + `openPlacementGate`（`repack-controller/pkg/nominate.go:596-610`）；为空则 `markPlacementGated`、gate 保持关闭（`:589-593`）。
- 调度器经 nomination 快速路径（§4.2.3「设计要点·调度器 nomination 快速路径」）校验并绑定；`validateNomination`（`allocate.go:976-1020`）任一项不过即回退正常梯度分配。
- 等 replacement pod 出现的那层保持：`Candidates` 要求 `Placement.ReplacementPodName` 非空，`reconcilePlacement` 在无候选时 requeue（间隔 `placementRetryInterval`，`placement_lease.go:439`）。

**不复算的后果（显式记录，均非正确性缺陷）**：

- **计划目标不可用时由调度器接管**：节点被删除、被写满或被 predicate 挡下 → nomination 校验不过 → 调度器回退梯度分配。此时 engine 侧的 `excludedFreedNodes` 排除（原 `placement_reconcile.go:104`，喂给 `:179`/`:228` 的 receiver 计算）已不存在，**replacement 可能落回本次刚腾空的节点** → `CompareFreedNodeSets` 报 missing → 终态 `ReasonBenefitNotRealized`（`decision.go:130-145`）。这是「engine 不再选点」的必然代价；该情形可观测、不会静默。
- **`==true` 单元的整组单域不丢**：调度器对 `RequiresHyperNodeAllocate()` 单元本就是**逐 HyperNode 整单元 dry-run + 单域提交**（`allocate.go:1156`、`:880-901`），gang 不会被撕散到多个域；退回的只是「域选得符不符合计划」。
- **漂移可观测**：实际绑定节点 ≠ `SelectedNodeName` → `outcomeCounts` 计入 `alternativeNodePlacements`（`decision.go:247-265`）→ 终态 `ReasonExecutionCompletedWithAlternativePlacement`（`:140-141`）。

**终局收口不变**：placement 长期未绑定仍由 run 级 `ExecutionDeadline` 收口——engine 首次落批 eviction 时打 `Status.ExecutionDeadline`（`eviction_reconcile.go:166-168`），各 requeue 以 `capAtExecutionDeadline` 封顶（`:441-445`），deadline 到达经 `timeoutExecution`（`execution_timeout.go:43-128`）把全部非终局 placement 统一置 `PodPlacementTimedOut`（`:119`）、整 run 以 `ReasonExecutionTimedOut` 失败（`:128`）。成员级 `ExpirationTime` 已随 wcx 特性合入移除（§7.2）。

**随本决策删除**：`==true` 整组就绪 gate、整组单域复算、按 PodGroup 的分组键，以及 receiver 决策的全部辅助——`unitReceiverUnion`、`unitPlannedDomainReceivers`、`smallestHyperNodeCovering`、`tierOfHyperNode`、`groupRelocationCandidates`、`relocationGroupReady`、`retainedRelocationsByPodGroup`，与 `executor/placement.Receivers`；`placement_reconcile.go` 中 `snapshot.FeasibleRelocation` 的调用全部移除（计划期调用不受影响）。`==true`/`==false` 在 Execute 侧不再分流。

**历史注记（原 §4.2.4 设计，已被本决策取代）**：本版之前，replacement 相按 `==true`/`==false` 双路径处理——`==true` 单元走「整组就绪 gate（journal 待决策成员集与已观测成员集相等前不写任何成员）→ 整组一次 `FeasibleRelocation` 求单一共置域 → 整组批量放行」，`==false` 走逐 pod 贪心；就绪期成员缺失或整组不可行即整组 hold 到 run 级 `ExecutionDeadline`。该设计修掉了「逐颗独立择域把整组拆散到多个 tier-1 域」的缺陷（原缺陷机制与 E21 红/绿回归见 §5.2.2），代价是把「选点」职责留在了 engine 侧、并引入第三个决策点。本版按「plan 即选点」整体移除该层。

**成员来源（易误解点，仍成立）**：不是「任一次 reconcile 只见部分成员」。每 pass 的 `reconcilePlacement` 以 durable journal `run.Status.Relocations` 为输入——Execute-prepare 时经 `PrepareExecuteRelocations`→`BuildPodRelocations`（`status/projection.go:139`、`:313-363`）为 plan 每颗 move 建**一条** relocation、在逐 pod eviction 前一次性持久化，每条以 `Phase=WaitingForReplacement` 起步（`status/projection.go:363`）；eviction 后仅被拒的 relocation 经 `retainSuccessfulRelocations` 裁剪（`internal/engine/eviction_journal.go:125-135`）。`Candidates`（`decision.go:33-51`）只是「已被 Nominator 认领、替换 pod 存活且未绑定」的子集——Candidates 是观测视角、**journal 才是成员来源**。
...
- **R18 每 gang 单元梯度收窄 + 逐域整体试放（规划期）**：接收者收窄在 `FeasibleRelocation` 内按 **gang 单元**复用梯度（§4.2.3 步骤 3：Job 单元调 `HyperNodeGradientForJobFn` 一次；**SubJob 单元调 Job 入口 + `HyperNodeGradientForSubJobFn` 两个入口取交集**——Job 入口兜底不继承的 Job 级拓扑）得允许域清单 `allowed`，**对命中 `RequiresHyperNodeAllocate()` 的单元**（谓词严格宽于「有硬需求」：含 SubJobPolicy / soft 拓扑 / 仅 Preferred 项单元，§4.2.3 步骤 3）**整单元落点限定在一个允许域的子树内**（逐域试放、first-fit、不评分，域内逐 pod 与 `SimulatePredicateFn` AND，§4.2.3 步骤 4；仅 `==false` 单元走贪心跨域、不锁定域，§4.2.3 步骤 3 前置判定）；**Execute 侧不复算**——replacement 相不调 `FeasibleRelocation`，直接把计划目标提名出去、落点校验交给调度器（§4.2.4），故本条为**规划期**约束。**（「两侧一致」不隐含「调度器未被修改」：Execute 侧真实调度经调度器的 nomination 快速路径把 subJob 钉到接收域——该机制为设计外、信任假设与「为何不补硬梯度复核」记录于 §4.2.3 设计要点·调度器 nomination 快速路径，本条「两侧一致」仅指 repack 的规划/执行共享注入点。）**
  - 验证：代码审查确认规划期 drain（`drain.go:378`）走 `FeasibleRelocation` 的「梯度收窄 + 逐域试放 + `SimulatePredicateFn` AND」实现；Execute 侧已无对应调用点（§4.2.4），「两调用点路径一致」这一断言面随之消失。约束保持的端到端正确性**由 e2e 兜底**（E8/E10/E12 走真实调度、不被 Execute 拒绝）。
...
- **R20 硬 tier 保持 + 锚点语义**：单元落点限定在该 gang 计划态梯度的允许 HyperNode 子树内——硬网络拓扑由 `network-topology-aware` 梯度 fn 保证（锚定：有剩余 pod → 收窄到最高允许祖先子树内；**整体腾空 → 无锚点、仅 `tier ≤ HighestTierAllowed` 生效、集群范围落位**（**SubJob 单元此句仅在本次调用覆盖整 job 时字面成立**：多 subJob job 中单个 subJob 整体腾空、兄弟 role 仍有残留 pod 时，Job 入口仍锚定非空 `job.AllocatedHyperNode`、交集后重新收窄回剩余 subJob 包络，与真实调度逐字节一致、非缺陷；本次调用覆盖整 job（各 role 全被逐出）时 Job 入口改取「保留者 gang 的 LCA」、无保留者才无锚，见「设计要点·允许域求值细节」）——规划期无锚分支不会自然触发（计划态承载下重算锚点恒非空，H1），实现须对整体腾空单元**显式临时清空 `AllocatedHyperNode` 再求梯度**（保存-恢复、见「设计要点·允许域求值细节」）；无锚分支经 `isEligibleHyperNode` 按插件实例 `hyperNodeResourceCache` 预过滤（`network_topology_aware.go:679-692`），缓存反映真实集群、不含计划内腾出容量，允许集**欠包含、保守**，会漏部分「计划后才可行」的域、但不产生违背约束的方案；另有 **drain 接收者池**在 pass 开始时即固定、只收 `TargetResourceNodePartial` 节点、显式排除已排空节点，两个保守来源叠加（见「设计要点·已知保守性」））；SubJob 级 = Job 入口 ∩ SubJob 入口（§4.2.2 小结表）：Job 入口以 Job 的 `NetworkTopology` 兜底（**SubGroupPolicy 分支不继承 Job 拓扑**，Job 级约束只在此表达，§4.2.2 第（1）点），SubJob 入口以该 SubJob 自身的 `NetworkTopology`/`AllocatedHyperNode` 叠加。
...
**Execute 期放置（执行环节，§4.2.4）**

- **R28 Execute 期直接把计划目标写为 `SelectedNodeName`、不复算接收者（§4.2.4）**：replacement 相对 `Candidates`（`decision.go:33-51`）解析出的每条 relocation 写 `SelectedNodeName = PlannedNodeName`（一次批量落盘 `writePlacementSelection`），Nominator 镜像到 `pod.status.nominatedNodeName` 并开 placement gate（`repack-controller/pkg/nominate.go:596-610`）。**Execute 侧不调 `FeasibleRelocation`**、不做任何可行域/接收者计算，`==true` 与 `==false` 不再分流；`==true` 单元的整组单域交由调度器原生语义保证（逐 HyperNode 整单元 dry-run + 单域提交，`allocate.go:1156`、`:880-901`），engine 不再介入域选择。计划目标不可用时调度器回退梯度分配，engine 侧的 `excludedFreedNodes` 排除随之失效——replacement 可能落回本次腾空的节点、终态 `ReasonBenefitNotRealized`；实际落点与计划不一致经 `alternativeNodePlacements` 计入 `ReasonExecutionCompletedWithAlternativePlacement`（`decision.go:247-265`、`:130-145`）。placement 长期未绑定仍由 run 级 `ExecutionDeadline` 收口（`eviction_reconcile.go:166-168`/`:441-445` → `execution_timeout.go:119`/`:128`）。
  - 验证：单测——reconcile 对每条候选写入的 `SelectedNodeName` 恒等于 `PlannedNodeName`（含「计划目标已被写满」「计划目标已从快照消失」的构造，断言仍照写、不做替换）；无候选时 requeue、不写任何成员；run 级 deadline 到达后非终局 placement 统一置 `PodPlacementTimedOut`、run 以 `ReasonExecutionTimedOut` 失败终局（`TestTimeoutExecutionFinalizesEveryRelocationPhase`/`TestExecutionDeadlineStopsFurtherEvictionAndMarksRunFailed`，`eviction_test.go:362/:311`）；e2e E21。
  - 覆盖现状注记：本条取代原「整组就绪 + 整组单域原子提交」规则（历史见 §4.2.4「历史注记」），其 helper 级单测锚点（`TestRelocationGroupReady` / `TestRetainedRelocationsByPodGroup` / `TestGroupRelocationCandidates` / `TestPodGroupPlannedNodesVisible` / `TestUnitReceiverUnion` / `TestUnitPlannedDomainReceivers`，`placement_reconcile_test.go`）随实现一并删除。
...
- **e2e（kind，复用 US-01 的 tier-tree helper 与 `npuFixture`/`occupy` 基建）**：新增 `test/e2e/repack/repack_topology_constraint.go`（同包 `repack`，Ginkgo v2 `Describe`），场景 E8/E9/E10a/E10b/E11/E12/E13/E16/E17/E18/E21/E22/E23/E24/E25（E14/E15 随共同放置组撤销而删除）：
  ...
  - **E21 `==true` 整组单域（Execute 环节端到端，§4.2.4）**：hard tier-1 约束多副本 gang（`networkTopology: {mode: hard, highestTierAllowed: 1}`，2 副本 × 双卡）整组作为 victim 迁移，接收容量刻意不对称（rt-s0 仅余恰装 1 颗的容量、rt-s1 可收整组）——断言替换 pod **共置单一 tier-1 hypernode**（全落 rt-s0 或全落 rt-s1）、RepackRun ExecutionCompleted、不被真实调度拒绝。对应 R28 + US-02 验收「不违背 PodGroup 网络拓扑约束」的 Execute 环节。**语义归属注记**：本用例原是「replacement 逐颗 first-fit 择域把整组拆散」缺陷的红/绿回归；§4.2.4 移除 Execute 侧复算后，整组单域不再由 engine 保证，而由「规划期整单元单域试放（§4.2.3 步骤 4）+ 调度器原生逐 HyperNode 整单元 dry-run、单域提交（`allocate.go:1156`、`:880-901`）」两侧共同保证——断言不变，红/绿的「红」侧已不可达（engine 不再有独立择域路径）。**另置「整组或零」子断言**：rt-s0 / rt-s1 各只留恰装 1 颗的容量（合计恰够一次跨域拆分的整组容量——无单一域装得下整组、但拆分在容量上本可行）→ 断言 RepackRun **规划期**不为该 gang 计划任何 move、不 freed 任何节点、无任何 Relocations（engine 放弃整理、gang 维持调度器原状，绝不拆散）；该保证来自 §4.2.3 步骤 4 的整单元单域试放、与本版 Execute 侧改动无关，拆分在容量上可行故拒绝非「无处可放」的平凡成立，而是真正钉住「绝不拆散」。
  - **E22/E23 整 job 腾空的 Job 入口锚点（H1 多 gang 修正，§4.2.3·允许域求值细节）**：Job 级与 role 级同为硬 tier-1、pod 全落在源域单节点的作业被整体驱逐、源域只剩被逐出的源节点——E22 是单 role（单单元）、E23 是两个 role（同一次规划的两个单元，各自一个 SubGroupPolicy）→ 断言 RepackRun `ExecutionCompleted`、源节点被腾空、replacement 全落在接收域（E22 单 role 落 rt-s1；E23 两个 role 同在 rt-s1，不得跨域分裂）。对应 R20 锚定语义的「整 job 腾空」分支；**E23 是 `jobFullyVacated` 按 call 判定的红/绿回归**：修复前 Job 入口保持驱逐前锚点、每个单元都不可行 → plan 恒为 nil、上报 `InsufficientImprovement`（实测：预修复镜像跑 E23 得到该 reason、修复后 `ExecutionCompleted`）。
...
- 实现 PR 的验收描述中**逐项勾选 R18–R28**，且 §5.2.2 所列 e2e 场景（E10 分 E10a/E10b，E14/E15 已随共同放置组撤销删除，E21 为 Execute 环节整组单域的端到端验收）**全部通过**，任一不满足视为 §4.2 未落地。
...
- **整 job 腾空时 Job 入口锚点同清（`jobFullyVacated`，本次改动）**：`allowedDomainsForTrial`（`domain_relocation.go`）原只临时清空**单元自身**的锚点，**SubJob 单元**且整 job 腾空时 `job.AllocatedHyperNode` 仍是驱逐前值（非空）——Job 入口据其把允许域收窄回**源域子树**，而该子树的节点集恰只含被逐出的源（接收者池只收 `TargetResourceNodePartial` 且排除源节点）→ 域内无接收者 → 单元恒不可行 → plan 恒为 nil、上报 `InsufficientImprovement`（用户报告：「没有可行的碎片整理方案」）。触发几何：每个 HyperNode 一个节点、作业整体落在一个 HyperNode、groupPolicy 与 rolePolicy 同为该层硬拓扑（Job 入口与 SubJob 入口都硬、两个作业对称 → 两侧都无候选）。修复第一版：新增 `jobFullyVacated`（整 job 的计划态已分配任务全为本次 victim），判中则连同 Job 入口锚点一并清空，仍走同一 Save/Restore。多 subJob job 中单个 subJob 整体腾空（Job 入口有残留 pod、锚点非空）行为不变——该场景真实调度同样按非空 job 锚点收窄，逐字节一致（§4.2.3「允许域求值细节」；该几何下「锚点 tier > 声明 tier」的收窄属不可达几何：满足「同一 tier ≤ T 的 HN 内」约束的 gang，其 pod 的 LCA tier 恒 ≤ T）。单测 `TestDomainRelocation_FullyVacatedJobClearsJobAnchor`（该几何，修复前 fail）、`TestDomainRelocation_FullyVacatedSubJobJoinsSurvivingSiblingDomain`、`TestDomainRelocation_PartialVacateKeepsSiblingDomainAllowed`（钉住「仅整单元腾空才改变包络」：残留 pod 时驱逐前锚点与按存活 pod 重算的锚点解析到同一包络、兄弟域恒在允许集内）；e2e `E22`（端到端：修复前 `InsufficientImprovement`、修复后 `ExecutionCompleted` 且 replacement 落在 rt-s1）。
  - **E24 多 role job 不得跨域分裂（同一次规划内的钉定，§4.2.3·允许域求值细节）**：E23 的几何但源域仍留一个空位——rt-s0 与 rt-s1 各能接纳一个 role、谁都不能同时容纳两个，于是唯一「能搬」的方案就是把 hard tier-1 job 拆到两个域 → 断言 `InsufficientImprovement`、0 节点腾空、0 move、两个 role 都留在源域。**此 spec 只守结果、不守机制**：实测把钉定关掉（`jobAnchorAfterEvacuation` 恒返回空）重编引擎后，E24 与 E23 都仍然通过——即「保留者 gang 的 LCA」这条钉定在 e2e 层面观察不到差别，它的判别者是单测 `TestDomainRelocation_MultiSubJobCoLocationRejectsSplitPlan`（建在 `registerHardTierGradient` 桩上）。**已定位（2026-09-21 聚焦取证，`repack_log_level: 5` 的引擎日志）**：naive 写法在 E24 几何里被**梯度侧的资源预过滤**先一步拒绝，与钉定无关——实测两个单元在其 `jobAnchor=""`/`subJobAnchor=""`（整 job 腾空、锚点确已清空）时允许域仍只有 `tier-1: rt-s0`（`domain_relocation.go:245`）：无锚路径经 `isEligibleHyperNode` 按 `job.GetMinResources()`（= PodGroup `Spec.MinResources`，`calcPGMinResources` 按 `MinAvailable=2` 求和 → 两个 role 共 4000）对**域级**资源缓存预过滤，而 rt-s1 的缓存 idle 只有 2000（n2 已占 6000/8000、n3 满）→ rt-s1 不进允许域；E23 的 rt-s1 idle 恰为 4000、刚好过闸，故 E23 能落 rt-s1。拒绝链（同一份日志）：单元 A 落 rt-s0 的 n1（`:212`）→ 单元 B 允许域仅 rt-s0、域内唯一接收者 n1 已被占 → `victim has no feasible receiver in domain`（`:397`）→ `unit INFEASIBLE — every allowed domain failed the trial`（`:219`）→ 排空单元 all-or-nothing 失败（`drain.go:380`）→ 0 move、`InsufficientImprovement`。（原先「trial 谓词栈拦住跨域落位」的猜测不成立：该插件不注册 predicate。）**结论**：E24 的几何对钉定无判别力（预过滤先挡掉 rt-s1），但钉定非冗余——当 rt-s1 的缓存 idle 被一个**空节点**抬到 ≥ `MinResources` 时（空节点不入 receiver 池 `eligibleReceiverNodes:555-557`、却计入域缓存 idle `network_topology_aware.go:126-131`），无锚的 naive 会把两个 role 拆到 rt-s0/rt-s1 两域、钉定则拒绝——桩级单测即此几何（无预过滤）。**能判别钉定的 e2e 形状**：E24 几何 + 一个「计入域 idle、却不入 receiver 池」的节点——现有 4 节点树即可构成（把 rt-s1 的第二个节点**留空**而非占满），见下条 E25。
  - **E25 源域仍有空位时也不得跨域分裂（保留者锚的判别 spec，§4.2.3·允许域求值细节）**：E24 的几何，但 rt-s1 的第二个节点**留空**——空节点不入 receiver 池（`eligibleReceiverNodes`，`drain.go:555-557`）却计入 rt-s1 的域级资源缓存 idle（`network_topology_aware.go:126-131`：n2 的 2000 + 空节点 8000 = 10000 ≥ 两个 role 的 `MinResources` 4000），于是**域级预过滤放行 rt-s1**、拒绝只能来自锚点。断言同 E24（`InsufficientImprovement`、0 腾空、0 move、两个 role 留在 rt-s0）。**取证（2026-09-21 聚焦跑三次，`repack_log_level: 5`）**：
    - 现状引擎 **绿**（18.9s）：单元 task-a `allowed="tier-1: rt-s0, rt-s1"`（确证预过滤放行了 rt-s1）→ 落 rt-s0 的 n1；单元 task-b `jobAnchor="rt-s0"`（保留者 gang 的 LCA）→ `allowed="tier-1: rt-s0"` → 域内唯一接收者已被占 → `victim has no feasible receiver in domain`（`:397`）→ `unit INFEASIBLE`（`:219`）→ 0 move。
    - 探针 A（禁用 `jobFullyVacated` 分支，即回到「不重写 Job 锚」的旧写法）**仍绿**：计划态同步出的 Job 锚（源域 rt-s0，`SyncJobAllocatedHyperNode` 按计划态全部任务取 LCA）恰好同样拒绝分裂——**E25 判别不了「重写」本身，那条由 E23 守**（整体腾空时旧写法留的是过期源域锚，E23 才会红）。
    - 探针 B（`subJobKeepsPods` 不再把「已被本计划落位」的 pod 计入保留者，即拆掉 `DisplacedTasks` 那一支）**红**：单元 task-b `jobAnchor=""` → `allowed="tier-1: rt-s0, rt-s1"` → rt-s0 试放失败后**落 rt-s1 的 n2**（`:212`）→ `moves=2`、源节点被腾空、run 以 `ExecutionCompleted` 收尾（非 `InsufficientImprovement`）→ 断言在 `completeReason` 处失败。**再跑一次带落点倾倒的探针**（临时把域断言提到 `completeReason` 之前）拿到终态：两个 role 实落 `integration-worker2/rt-s0` 与 `integration-worker3/rt-s1`——**分裂真的落到集群里、执行期没有任何一层拦住它**（`selectedNodePlacementCount=2`、`timedOutPlacementCount=0`、`phase=Succeeded`）。故规划期的保留者锚是这条 hard tier-1 约束的**唯一关口**，工具侧（nomination / 落点校验）不兜底。
    - 结论：E25 守的是「保留者锚（**含本计划已落位的兄弟 role**）存在且指向领先者域」这条不变式；E24 因预过滤先挡掉 rt-s1 而看不见它。
    - **注**：含 E25 的 **96-spec 全量已于 2026-09-21 重跑全绿（2937.8s ≈ 48m58s，0 failed / 0 skipped）**；上文「95 specs 全绿」那次不含 E25。
  - **后续修正（本次改动）**：第一版按**单元**判整 job 腾空，只覆盖单 role job——多 subJob job 中「每个 role 各成一个单元」，任一单元都不覆盖整 job → 判据恒假 → Job 入口保持驱逐前锚点（源域子树、域内无接收者）→ 每个单元都不可行、plan 恒为 nil（用户报告：「只考虑了一个 job 有一个 subjob 的情况」，E22 几何换成两个 role 即复现）。改为**按 call 判**（`jobFullyVacated(scope, unit)`，victim 集取本次 `FeasibleRelocation` 全部单元）＋**Job 入口锚点重写为「保留者 gang 的 LCA」**而非无条件清空（`jobAnchorAfterEvacuation`；保留者 = 未被本次调用逐出、或已被本计划落位——由新增 `DisplacedTasks` 读计划态 `rewritten` 判定）：原「一律清空」会把兄弟 role 释放到另一个 tier-1 域，把 hard tier-1 job 计划成跨域分裂；无保留者时置空，与第一版（E22 几何）逐字节一致。接口上 `ClearGangAnchor` 泛化为 `SetGangAnchor(job, subJob, value)`；单测 `TestDomainRelocation_MultipleSubJobsVacatedTogetherLeaveSourceDomain`（E22 几何的双 role 版，修复前 fail：`fit=false`）、`TestDomainRelocation_MultiSubJobCoLocationRejectsSplitPlan`（钉住跨域分裂被拒：去掉钉定即 `fit=true` 分裂两域）、`TestSessionPlanState_SetGangAnchor`、`TestJobFullyVacated_CallScopeMembership`（判据本身的四条分支：整 job 在本次调用内 / 兄弟 gang 留存 / job 无 allocated pod / job 不存在）；e2e `E23`（同几何端到端：修复前无 plan、修复后 `ExecutionCompleted` 且两个 role 的 replacement 同在 rt-s1）与 `E24`（不可分裂的端到端不变量，判别力说明见 §5.2.2 的 e2e 清单）；`E25`（E24 几何 + rt-s1 留空节点，域级预过滤不再遮挡 → **保留者锚本身有判别力**：拆掉 `DisplacedTasks` 那一支即红、现状绿；只禁用 Job 锚重写仍绿，即「重写」本身由 E23 守，见 §5.2.2）。
  - 性质：影响正确性（plan 侧假阴性，锚点清空判据）。

- **允许域清单层内排序（`sortLayersByName`，本次改动）**：`allowedDomains` 返回的 `allowed` 直接取插件梯度函数的返回序——**层间**由插件按 tier 升序重排（`network_topology_aware.go:655-665`），**层内**（同 tier 各域）却是 BFS 遍历 `current.Children`（map 迭代，`:646`）的入列序，随运行变化；步骤 4 的 first-fit 取清单首域，不排序即**同一份输入在不同轮次选到同 tier 的不同域、计划不可复现**（实测：同输入 200 次调用出两种 tier-1 序、179/21）。修复在 adapter 侧收口——`allowedDomains` 的两个返回点都经 `sortLayersByName`（`domain_relocation.go:139-147`）对每个 tier 层按 HyperNode 名升序排序；插件不动（调度器侧域选择由 `HyperNodeOrderFn`/评分决定、与层内序无关，改插件会波及真实调度路径）。单测 `TestAllowedDomains_SortsDomainsWithinTier`（反向注入同层 `hD,hB,hA`、断言输出 `hA,hB,hD`；去掉排序即失败）。**注**：E23/E24 的既有 e2e 结论取自排序前的引擎；含本条排序的引擎已于 2026-09-21 重跑**全量 repack e2e（95 specs 全绿，49m10s，镜像由当前源码重编并核对过符号）**，E22/E23/E24、E21 两子断言与「==true whole-gang reconcile」等拓扑约束 spec 全部通过——层内序的改变没有改变这些几何的终态（E24 的拒绝点其后由一次聚焦取证定位：是梯度侧的域级资源预过滤、与层内序无关，见下条 `jobFullyVacated` 的 E24 注记）。
  - 性质：影响计划可复现性（同一输入的计划在不同轮次可能不同）；不改约束语义。
...
- **cross-unit full-vacate 保守缺口（候选单元恒为单节点）**：排空单元恒为单节点（§2.4 设计约束），一个 gang 跨两节点、两次 drain 各属独立单元时，每个 per-node 单元的 victims 均覆盖不了整个 gang → H1（整体腾空临时清锚，及其按 call 判整 job 腾空的 Job 入口重写——一次 drain 调用的 victims 只覆盖单个节点）对该 gang 恒不触发，单元保持锚定、不逃逸源子树。结果保守（不产生违背约束的方案），但会漏「先整体腾空、再跨域落位」的可行整理。
...
**合并交互注记（wcx「PDB 重试 + eviction 截止」特性，§4.2.4/R28/E21 改写依据）**：wcx 提交 `0893b1dc3`–`e15602086`（PDB 阻塞驱逐加 deadline 重试 / 驱逐重试生命周期加固 / 零扰动 PDB plan 过滤 / run 删除时释放执行槽）合入本需求上游后，Execute 期有界性从**成员级 `ExpirationTime`** 整体替换为 **run 级 `ExecutionDeadline`**：`ExpirationTime`、`nominationTTL`、`CanExpire`、`expirePlacements` 与其消费代码全部移除（代码零残留）；`PodPlacementTimedOut` 的唯一写入者变为 `timeoutExecution`（`execution_timeout.go:119`）。新语义：engine 首次落批 eviction 时打 `Status.ExecutionDeadline = now + ExecutionTimeout`（`eviction_reconcile.go:166-168`），各 requeue 以 `capAtExecutionDeadline` 封顶（`:441-445`），deadline 到达经 `timeoutExecution`（`execution_timeout.go:43-128`）把全部非终局 placement 统一置 `PodPlacementTimedOut`（`:119`）、整 run 以 `ReasonExecutionTimedOut` 失败（`:128`）。
  - 对 §4.2.4 的后果：placement 等待不再有成员级 TTL——整 run 共享同一 deadline、同时到期；原「近同时 TimedOut、gate 全开、交还 volcano-scheduler 以原生 `==true` 单域语义续调度整组」的逃生**不再可达**——engine 放弃后 run 以 `ReasonExecutionTimedOut` 失败终局收口，而非把 gang 交还调度器续跑。§4.2.4/R28/E21 已按合并后语义改写。
  - 性质：合并交互注记（行为记录，非本需求缺陷）；deadline 终局表现已由 repack e2e 验收（`placement_protocol.go`「nominates an occupied planned receiver, lets the scheduler override it, and fails on the unrealized plan」：deadline 到达后 run 以 `ExecutionTimedOut` 失败收口，placement gate 与 PodGroup lease 均按期释放）。

**设计决策注记：replacement 相「计划即选点」（§4.2.4 / R28 / §5.2.2 E21 改写依据）**：replacement 相原在 `FeasibleRelocation` 上按 `==true`/`==false` 双路径复算接收者（`==true` 整组就绪 + 整组单域原子提交），本版决定**整体移除该层**——Execute 期直接把 plan 的 `Move.ToNode` 写为 `SelectedNodeName`，落点校验交给调度器。
  - 理由：复算与计划期用的是同一个 `FeasibleRelocation`（只是快照更新），使同一份计划有三个可改答案的决策点；复算提供的「计划目标不可用时改选其它节点」韧性本身即与计划不一致，且并未消除真实调度这一层、只是把偏差从 scheduler 侧挪到 engine 侧。移除后决策点降为两个、权责单一：**plan 选点、scheduler 绑定**。
  - 代价（显式接受）：engine 侧的 `excludedFreedNodes` 排除随之失效——计划目标不可用时调度器回退梯度分配，而 binpack 偏好刚腾空的源域（`allocate.go:667-671` 注释），replacement 可能落回本次腾空的节点 → `ReasonBenefitNotRealized`。该情形可观测、不静默。`==true` 整组单域不丢（由调度器原生逐 HyperNode 整单元 dry-run + 单域提交保证）。
  - 不涉及的 API 变更：字段、枚举、终态语义均不变，`PodPlacementNominated` 仍由 Nominator 写入（`nominate.go:1446`）、由 `status/store.go:83/:129/:133` 读取；无需 codegen / manifests。
  - 过时注释已同步（行号不变，仍为 `nominate.go:590-592`）：原注释「Hand the placement decision to the engine, which owns the scheduler session and can choose a current receiver」随本决策语义过期（engine 已不再选点、不再持有 placement 期的 scheduler session），已改写为「Wait for the engine to write the planned receiver as SelectedNodeName；下一次 reconcile 提名之，落点校验交给调度器」。行为本身（`markPlacementGated` 的调用时机与语义）未变；主模块 `vendor/volcano.sh/repack-controller`（gitignored 的本地构建产物）副本同步。`repackrun_types.go` 的 `SelectedNodeName`/`PlannedNodeName` 字段注释按决策**不修改**（已确认）。
  - 性质：设计决策记录（行为面收窄 + 复杂度下降）。
