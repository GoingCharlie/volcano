...
| Planning | nine consolidation layouts, lower-disruption migration direction, whole-PodGroup movement with `minAvailable=1`, static zero-disruption PDB full/partial filtering and relaxation, informer synchronization, empty-node exclusion |
...
| Execute result | success, verified no-op, static PDB filtering, dynamic PDB rejections and plan/result separation, exact planned/actual freed-node mismatch, zero-node `PartiallySucceeded` projected through `Complete/BenefitNotRealized` |
...
terminal condition reason. `PlacementTimedOut` and `ResultVerificationFailed`
are failed Warnings. `BenefitNotRealized` is a `PartiallySucceeded`
Warning after execution completes without an error but not every planned node
is verified free. `ExecutionCompletedWithAlternativePlacement` is successful
when every replacement is scheduled and every planned freed node is verified.
