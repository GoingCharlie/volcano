...
func TestClassifyEvictionError(t *testing.T) {
	...
	cases := []struct {
		...
		{"ErrVictimNotFound is victim gone", engineframework.ErrVictimNotFound, evictionVictimGone},
	}
...
// TestExecuteEvictionWavePartialAcceptanceRoutesToPlacement: 3 Accepted + 2 PDB
// 429 -> accepted go to placement, retryable stay InProgress, run not terminal.
func TestExecuteEvictionWavePartialAcceptanceRoutesToPlacement(t *testing.T) {
	fixedNow := time.Now()
	engine := newWaveTestEngine(fixedNow)
	run := waveTestRun(
	...
	err := engine.executeEvictionWaveWithClient(context.Background(), run.DeepCopy(), 0, "example.com/accelerator", kubeClient)
	if err != nil {
		t.Fatalf("executeEvictionWaveWithClient: %v", err)
	}
	if attempts != 5 {
