...
func TestBuildPodRelocations(t *testing.T) {
	if relocations, err := enginestatus.BuildPodRelocations(nil, time.Unix(100, 0), nil); err != nil || relocations != nil {
		t.Error("nil plan -> nil")
	}
	...
	relocations, err := enginestatus.BuildPodRelocations(plan, baseTime, testPodGroupPlacementPolicies{
		"ns/g": true,
	})
	if err != nil {
		t.Fatal(err)
	...
	homogeneous, err := enginestatus.BuildPodRelocations(plan, baseTime, testPodGroupPlacementPolicies{
		"ns/g": false,
	})
	if err != nil {
		t.Fatal(err)
	...
	if _, err := enginestatus.BuildPodRelocations(missingVictimPod, baseTime, testPodGroupPlacementPolicies{
		"ns/g": true,
	}); err == nil {
	...
	for _, testCase := range invalidPodGroups {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if _, err := enginestatus.BuildPodRelocations(
				invalidPlan, baseTime, testPodGroupPlacementPolicies{},
			); err == nil {
...
func TestApplyPlan(t *testing.T) {
	...
	if err := enginestatus.PrepareExecuteRelocations(exec, plan, time.Unix(100, 0), testPodGroupPlacementPolicies{}); err != nil {
		t.Fatal(err)
	}
