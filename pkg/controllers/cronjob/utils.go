...
func (o byJobStartTime) Less(i, j int) bool {
	// 如果 i 的 CreationTimestamp 是零值，而 j 的不是，i 应该排在后面（return false）
	if o[i].ObjectMeta.CreationTimestamp.IsZero() && !o[j].ObjectMeta.CreationTimestamp.IsZero() {
		return false
	}
	// 如果 j 的 CreationTimestamp 是零值，而 i 的不是，i 应该排在前面（return true）
	if !o[i].ObjectMeta.CreationTimestamp.IsZero() && o[j].ObjectMeta.CreationTimestamp.IsZero() {
		return true
	}
	// 如果两个 CreationTimestamp 相同，则按 Name 排序
...
func deleteFromActiveList(cj *batchv1.CronJob, uid types.UID) {
	if cj == nil || len(cj.Status.Active) == 0 {
		return
	}
